...
int main (int argc, char* argv[])
{
    ...
    case 2://INSERCAO DE N REGISTROS NA TABELA-HASH
    {
        ...
        for(int i=0; i<N; i++)
        {
            int key =buffer[i].codigoDaCidade+buffer[i].dataDeConfirmacao.dia+buffer[i].dataDeConfirmacao.mes+buffer[i].dataDeConfirmacao.ano;
    ...
    case 5://ANALISE DAS ESTRUTURAS BALANCEADAS
    {
        ...
        for(int i=0; i<np; i++)
        {
            int key =buffer_p[i].codigoDaCidade+buffer_p[i].dataDeConfirmacao.dia+buffer_p[i].dataDeConfirmacao.mes+buffer_p[i].dataDeConfirmacao.ano;
        ...
        if(saida.is_open())
        {
            ...
            for(int k=0; k<5; k++) //5 iteracoes para cada valor de N
            {
                ...
                for(int i=0; i<N[n]; i++)
                {
                   int key =bufferALEAT_p[i].codigoDaCidade+bufferALEAT_p[i].dataDeConfirmacao.dia+bufferALEAT_p[i].dataDeConfirmacao.mes+bufferALEAT_p[i].dataDeConfirmacao.ano;
