...
void Diretorio::Split(int numBalde)
{
    ...
    map<unsigned long long int, Covid19> temp;
...
void Diretorio::Insere(unsigned long long int key,Covid19 chave,bool reinsere)
{
...
void Diretorio::Remove(unsigned long long int key,int modo)
{
    int numBalde = Hash(key);
...
void Diretorio::Atualiza(unsigned long long int key, Covid19 chave)
{
   int numBalde = Hash(key);
    dir[numBalde]->Atualiza(key,chave);
}

void Diretorio::Procura(unsigned long long int key)
{
    int numBalde = Hash(key);
